...
# === Config ===
PARQUET_PATH = "./data/geographic_data/vegetation_embeddings.parquet"
OUTPUT_PATH = "./experiments/embedding_space/vegetation_embedding_space.png"
MAX_POINTS = 80000
POINT_SIZE = 20
ALPHA = 0.5
COLORMAP = "viridis"

df = pd.read_parquet(PARQUET_PATH, columns=["lat", "lon", "vegetation_embedding"])
if len(df) > MAX_POINTS:
    df = df.sample(MAX_POINTS, random_state=42).reset_index(drop=True)
coords = df[["lon", "lat"]].to_numpy()
embeddings = np.stack(df["vegetation_embedding"].tolist())
...
ax.set_title("Vegetation Embedding Similarity (1D PCA)")
ax.set_aspect("equal")
