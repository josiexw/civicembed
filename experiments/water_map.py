...
# === Config ===
PARQUET_PATH = "./data/geographic_data/water_embeddings.parquet"
OUTPUT_PATH = "./experiments/embedding_space/water_embedding_space.png"
MAX_POINTS = 80000
POINT_SIZE = 20
ALPHA = 0.5
COLORMAP = "cividis_r"

df = pd.read_parquet(PARQUET_PATH, columns=["lat", "lon", "water_embedding"])
if len(df) > MAX_POINTS:
    df = df.sample(MAX_POINTS, random_state=42).reset_index(drop=True)
coords = df[["lon", "lat"]].to_numpy()
embeddings = np.stack(df["water_embedding"].tolist())
...
ax.set_title("Water Embedding Similarity (1D PCA)")
ax.set_aspect("equal")
